...
def apply_indirect_cash_flow_fallbacks(
    ...
    if df_bs_calc is not None:
        ...
        if (
            ...
            df.loc["ChangeInAccountsReceivable", common_cols] = df.loc[
                "ChangeInAccountsReceivable", common_cols
            ].fillna(ar_diff)
        ...
        if "ChangeInInventory" in df.index and "Inventory" in bs_sorted.index:
            ...
            df.loc["ChangeInInventory", common_cols] = df.loc[
                "ChangeInInventory", common_cols
            ].fillna(inv_diff)
        ...
        if (
            ...
            df.loc["ChangeInAccounts_Payable", common_cols] = df.loc[
                "ChangeInAccountsPayable", common_cols
            ].fillna(ap_diff)
    ...
    if df_indirect_cf is not None:
        ...
        for date in df_indirect_cf.index:
            ...
            raw_cf_snippet = (
                ...
                if (raw_df_cf is not None and date_str in raw_df_cf.index)
                else {}
            )
...
for ticker in target_tickers:
    ...
    if source == "yfinance":
        ...
        dfCashFlowQ = pd.DataFrame(data["cf_q"]) if data["cf_q"] is not None else None
    ...
    audit_results_Y, clean_yearly_indirect_cash_flow = validate_financial_statements(
        ...
        ticker=ticker,
        # Pass the pre-mapped dataframes (transposed so dates are the index)
        raw_df_is=dfIncomeStatementY.T if dfIncomeStatementY is not None else None,
    ...
    # Do the same for Q statements if they exist
    if dfIncomeStatementQ is not None:
